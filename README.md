...
* **build_dataset_duplicate.py**: The main pipeline script. Parses OUTCAR/vasprun.xml files, handles the train/test split, and outputs a master summary CSV. Use the `--duplicate 1` flag to automatically supercell small geometries so they don't violate the minimum image convention based on the cutoff in the ffield file.
...
```bash
python train_without_force_Li_NN.py --s=10000 --lr=0.0001 --pr=200 --writelib=1000 --batch=50 --bo 0 --h 0 --a 0 --t 0 --f 0 --vdw 1
```
